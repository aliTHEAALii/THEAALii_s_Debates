//
//  TiPostUserButton.swift
//  THEAALiisDebates
//
//  Created by Ali Kadhum on 9/26/24.
//

import SwiftUI

struct TiPostUserButton_Previews: PreviewProvider {
...
//                            .frame(width: width * 0.12 * scale)
                        ...
                        if user != nil {
                            if user!.profileImageURLString != nil {
                                AsyncImage(url: URL(string: user!.profileImageURLString!), scale: 0.5 * scale) { image in
                                    image
                                        .resizable()
                                        .clipShape(Circle())
                                        .scaledToFit()
